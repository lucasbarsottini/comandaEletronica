<html lang="pt-BR">
  ...
  <body class="page">
    ...
    <header >
     ...
     <img src="{{asset('img/lanches/' . $produto->imagem)}}" alt="" class="imagem_logo">
     </div>
    </header>
    ...
    @foreach($produtos as $produto)
      <section class="container_principal">
        ...
        <div class="container_imagem">
          <img src="{{asset('img/lanches/x-tudo.png')}}" alt="" class="imagem">
        </div>
  ...
  <form action="{{ route('adicionar-lanche') }}" method="POST" class="form_container">
    ...
    <img src="{{ asset('img/lanches/remove.png') }}" alt="" type="button" class="input_menos" data-id="{{ $produto->id }}">
        ...
        <input type="hidden" data-id="{{$pedidos->id}}" name="idPedido" >
        @endforeach
